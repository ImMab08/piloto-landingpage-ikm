...
export function ClientsSection() {
  ...
  const recalcSizes = () => {
    ...
    const totalWidth =
      reviews.length * computedCardWidth + (reviews.length - 1) * GAP;
    ...
    >
      <div className="space-y-4">
        <div className="space-y-1 md:space-y-2">
          <div className="flex items-center text-accent space-x-1 md:space-x-2">
            ...
            <p className="font-oswald font-medium text-sm md:text-lg">
              {t("text")}
            </p>
          </div>
      ...
      <div className="relative flex flex-col w-full h-full">
        ...
        <div
          ...
          className={`relative w-full left-0 right-0 flex gap-5 md:mt-6 z-10 ${
        ...
        >
          {reviews.map((review, id) => (
            <div
              key={id}
              className="h-[400px] md:h-[430px] rounded-lg shadow overflow-hidden flex flex-col flex-shrink-0 duration-300 hover:scale-[1.01] md:hover:scale-[1.03] hover:shadow-2xl"
              style={{ width: cardWidth || undefined }}
            >
              <div className="w-full h-3 bg-primary"></div>
              <div className="flex-1 flex flex-col justify-between ">
                <div className="h-full flex flex-col space-y-5 py-10 md:py-14 px-6 md:px-10">
                  <div className="font-oswald">
                    <p className="font-medium text-2xl text-primary leading-6">
                      {review.author}
                    </p>
                    <p className="text-lg text-accent">{review.location}</p>
                  </div>
                  <p className="text-justify text-sm md:text-base text-text-tertiary leading-5 md:leading-6">
                    {review.text}
                  </p>
                </div>
                <div className="px-6 md:px-10">
                  <div className="flex justify-end border-border border-t py-5">
                    {Array.from({ length: review.stars }).map((_, id) => (
                      <IconStar
                        key={id}
